...
function a5_menuAddItem(type,text,textOver,icon,iconOver,href,js,html,childMenu,A5Menu){
...
function a5_menuRender(A5Menu){
	...
	renderedHTML = renderedHTML+'<div id="A5MENU_'+A5Menu.varName+'" class="'+A5Menu.style+'MenuCont" style="position: absolute; display: none;"><ul class="'+A5Menu.style+'Menu">\n'
	...
	for(i=0;i<A5Menu.arr.length;i++){
		...
		if(curMenuEle.type == 1){
			curRenderJS = ''
			curRenderIcon = ' style="padding-left: '+renderImgSize+'"';
			...
			if(curMenuEle.href != ''){
				curRenderJS = ' onclick="document.location = \''+curMenuEle.href+'\';"';
			}
			...
			if(curMenuEle.icon != ''){
				curRenderIcon = ' style="background-image: url(\''+curMenuEle.icon+'\'); background-position: center left; background-repeat: no-repeat; padding-left: '+renderImgSize+'"';
			}
			
			curContents = '<span'+curRenderIcon+' style="white-space: nowrap;">'+curMenuEle.text+'</span>'
			if(curMenuEle.childMenu != ''){
				curContents = '<span class="'+A5Menu.style+'MenuCas" style="display: block;">'+curContents+'</span>'
			}
			renderedHTML = renderedHTML + '\t<li'+curRenderID+' class="'+curRenderClass+'" style="width: '+A5Menu.width+';" onmouseover="a5_menuOnMouseOver('+A5Menu.varName+',this,'+i+','+A5Menu.TOvarName+')" onmouseout="'+A5Menu.TOvarName+' = a5_menuOnMouseOut('+A5Menu.varName+',this,'+i+')"'+curRenderJS+'>'+curContents+'</li>\n';
		}
			...
			curRenderClass = A5Menu.style+'MenuOff';
			renderedHTML = renderedHTML + '\t<li'+curRenderID+' class="'+curRenderClass+'" style="width: '+A5Menu.width+';" onmouseover="a5_menuOnMouseOver('+A5Menu.varName+',this,'+i+','+A5Menu.TOvarName+')" onmouseout="'+A5Menu.TOvarName+' = a5_menuOnMouseOut('+A5Menu.varName+',this,'+i+')">'+curMenuEle.html+'</li>\n';
		}
		else if(curMenuEle.type == 3){
			renderedHTML = renderedHTML + '</ul>\n\t<div class="'+A5Menu.style+'MenuSeparator" style="width: '+A5Menu.width+';" onmouseover="a5_menuOnMouseOver('+A5Menu.varName+',this,'+i+','+A5Menu.TOvarName+')" onmouseout="'+A5Menu.TOvarName+' = a5_menuOnMouseOut('+A5Menu.varName+',this,'+i+')">&nbsp;</div>\n<ul class="'+A5Menu.style+'Menu">';
		}
	}
	renderedHTML = renderedHTML + '</ul></div>\n'
	renderContainer = document.getElementById(A5Menu.container)
